...
# Copy only dependency-related parts of package.json to preserve cache
COPY package.json temp.package.json
RUN jq 'del(.version)' temp.package.json > package.json
...
RUN echo "Checking for lockfiles:" && \
    ls -la yarn.lock* package-lock.json* pnpm-lock.yaml* 2>/dev/null || echo "No lockfiles found"

# Split the build command for better error visibility
RUN \
  if [ -f yarn.lock ]; then \
